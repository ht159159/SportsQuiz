<template>
  <main class="edit">
    ...
    <transition name="fade">
      <div
        v-show="tan"
        style="width: 100%;height: 100%;background: rgba(0,0,0,.5);position:absolute;z-index:22;top: 0"
      >
        <div
          class="boxB"
        ...
        >
          ...
          <div class="bottom">
            <p style="line-height: .38rem;text-align: center;font-size: .26rem">
              {{ $t("SureDelete") }}
            </p>
            <p class="di">
              <span
                @click="quxiao"
                style="background: linear-gradient(to right, #c5c5c5, #acacac)"
                >{{ $t("Cancel") }}</span
              >
              <span
                @click="que"
                style="background: linear-gradient(to right, #e73a51, #dc3849);margin-left: .4rem"
                >{{ $t("Confirm") }}</span
              >
            </p>
          </div>
